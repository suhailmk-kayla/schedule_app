...
_setError(failure.message);
  ...
  Future<void> createTempOrder() async {
    ...
    // Local-only temp orders use negative orderId space to avoid clashing with server IDs.
    // Next local orderId: -1, -2, -3, ...
    final lastLocalEntryResult = await _ordersRepository.getLastLocalOrderEntry();
    int nextOrderId = -1;
    lastLocalEntryResult.fold(
      (_) {},
      (lastOrder) {
        if (lastOrder != null) {
  ...
  Future<bool> sendOrder(
    ...
    try {
      ...
      // EDIT path: existing order on server (not temp, not draft). Use update_order API like KMP EditOrder.
      // CREATE path: temp (2) or draft (3) — order not yet on server; use addOrder API.
      if (_orderMaster!.orderFlag != OrderFlags.temp && _orderMaster!.orderFlag != OrderFlags.draft) {
      ...
      // CREATE path: temp or draft order. Use addOrder API like KMP CreateOrderScreen.
      ...
      // Remove the local-only temp/draft order (negative orderId) and its subs.
      // This prevents any future ambiguity between the local placeholder and the server order.
      final int oldLocalOrderId = _orderMaster!.orderId;
      if (oldLocalOrderId < 0) {
        await _ordersRepository.deleteLocalOrderAndSubsByOrderId(oldLocalOrderId);
      }
  ...
  Future<bool> addProductToOrder({
    ...
    try {
      ...
      // Local-only temp/draft order subs use negative orderSubId space to avoid clashing with server IDs.
      // Next local orderSubId: -1, -2, -3, ...
      final lastLocalSubEntryResult = await _ordersRepository.getLastLocalOrderSubEntry();
      int orderSubId = -1;
      lastLocalSubEntryResult.fold(
        (_) {},
        (lastOrderSub) {
          if (lastOrderSub != null) {
            orderSubId = lastOrderSub.orderSubId - 1;
          }
        },
      );
  ...
  Future<bool> addProductToExistingOrder({
    ...
    try {
      ...
      // 2. Local-only inline new items use negative orderSubId space to avoid clashing with server IDs.
      // These IDs are never sent to the server as order_sub_id; server assigns real IDs and we replace
      // them in local DB from the API response.
      final lastLocalSubEntryResult =
          await _ordersRepository.getLastLocalOrderSubEntry();
      int defaultOrderSubId = -1;
      lastLocalSubEntryResult.fold(
        (_) {},
        (lastOrderSub) {
          if (lastOrderSub != null) {
            defaultOrderSubId = lastOrderSub.orderSubId - 1;
          }
        },
      );
